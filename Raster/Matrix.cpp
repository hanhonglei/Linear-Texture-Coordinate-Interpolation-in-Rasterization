	...
	for (int i = 0 ; i < m_nNumRows ; ++i)
	{
		for (int j = 0 ; j <  m_nNumColumns; ++j)
			if (result.GetElement(i,j)!=0)
				result.SetElement(i, j, pow(result.GetElement(i, j),value));
	}
...
CMatrix	CMatrix::operator % (int value) const
{
	// 构造目标矩阵
	CMatrix	result(*this) ;		// copy ourselves
	// 进行数乘
	...
	{
		for (int j = 0 ; j <  m_nNumColumns; ++j)
			result.SetElement(i, j, (int)result.GetElement(i, j) %  value) ;
	}
...
{ 
    ...
    { 
        ...
        if (fm<eps)
		{
			...
			//SortEigenVal(dblEigenValue,mtxEigenVector,0,m_Abs,0,m_nNumColumns-1);
			return TRUE;
		}
...
Loop_2: 
    ...
    { 
		...
		w=i*m_nNumColumns+q;
        fm=mtxEigenVector.m_pData[u];
        mtxEigenVector.m_pData[u]=(fm*cn+mtxEigenVector.m_pData[w]*sn);//修改：取相反
        mtxEigenVector.m_pData[w]=(-fm*sn+mtxEigenVector.m_pData[w]*cn);//修改：取相反
	}

	goto Loop_1;
}
//mode=2 按行进行均值求解,以列形式返回
//mode=1 按列进行均值求解,以行形式返回
//vec=TRUE 返回均值向量
//vec=FALSE 返回均值矩阵
CMatrix CMatrix::GetMean(int mode,BOOL vec)
{
	CMatrix result;
	if (vec)
	{
		if (mode==1)
			result.Init(m_nNumRows,1);
		else
			result.Init(1,m_nNumColumns);
	}
	else
		result.Init(m_nNumRows,m_nNumColumns);
	LONG i,j;
	double sum,avg;
	if (mode==1)
	{	//按列求均值(样本矩阵以列向量形式存储)
		for (i=0;i<m_nNumRows;i++)
		{
			...
			for (j=0;j<m_nNumColumns;j++)
				sum+=GetElement(i,j);
			avg=sum/m_nNumColumns;
			if (!vec)
			{
				for (j=0;j<m_nNumColumns;j++)
					result.SetElement(i,j,avg);
			}
			else
				result.SetElement(i,0,avg);				
		}
	}
	else
	{
		...
		for (i=0;i<m_nNumColumns;i++)
		{
			...
			{
				for (j=0;j<m_nNumRows;j++)
					result.SetElement(j,i,avg);
			}
			else
				result.SetElement(0,i,avg);				
		}
	}
	return result;
}
void CMatrix::SetRow(int nRow,double value[])
{
	int j;
	for (j=0;j<m_nNumColumns;j++)
		SetElement(nRow,j,value[j]);
}
void CMatrix::SetCol(int nCol,double value[])
{
	int j;
	for (j=0;j<m_nNumRows;j++)
		SetElement(j,nCol,value[j]);
}
...
void CMatrix::SortEigenVal(double EigenValue[], CMatrix &EigenVector,int mode,int left, int right)
{
	int i, last;
	if (left >= right) return;
	double *col1=(double*)new double[EigenVector.m_nNumRows];
	double *col2=(double*)new double[EigenVector.m_nNumRows];
	EigenVector.GetColVector(left,col1);
	EigenVector.GetColVector((left+right)/2,col2);
	EigenVector.SetCol((left+right)/2,col1);
	EigenVector.SetCol(left,col2);
	swap(EigenValue, left, (left+right)/2);
	last = left;
	for (i = left+1; i <= right; i++)
	{
		if (mode==0)
		{
			if (EigenValue[i] > EigenValue[left])
			{
				EigenVector.GetColVector(i,col1);
				EigenVector.GetColVector(++last,col2);
				EigenVector.SetCol(i,col2);
				EigenVector.SetCol(last,col1);
				swap(EigenValue, last, i);
			}
		}
		else
		{
	...
	EigenVector.SetCol(left,col2);
	EigenVector.SetCol(last,col1);
	swap(EigenValue, left, last);
	if (col1) delete [] col1;
	if (col2) delete [] col2;
	SortEigenVal(EigenValue, EigenVector,mode,left,last-1);
	SortEigenVal(EigenValue, EigenVector,mode,last+1,right);
}

CMatrix CMatrix::Dot(const CMatrix &other) const
{
	// 首先检查行列数是否相等
	...
	for (int i=0;i<m_nNumRows;i++)
	{
		for (int j=0;j<m_nNumColumns;j++)
		{
			result.SetElement(i,j,GetElement(i,j)*other.GetElement(0,j));
		}
	}
	return result;
}

void CMatrix::GenIdentity(int nRows, int nColumns)
{
	int i,j;
	m_nNumRows=nRows;
...
BOOL CMatrix::MakeDiag(int nSize, double *v)
{
	if (! Init(nSize, nSize))
		return FALSE;
...
CMatrix CMatrix::GetRows(LONG nRows1,LONG nRows2)
{
	CMatrix result;
	double *v=new double[m_nNumColumns];
	int i=0;
	ASSERT(nRows2>nRows1 && nRows2<=m_nNumRows);
	result.Init(nRows2-nRows1,m_nNumColumns);
	for (i=nRows1;i<nRows2;i++)
	{
		GetRowVector(i,v);
		result.SetRow(i-nRows1,v);
	}
	if (v) delete [] v;
	return result;
}

CMatrix CMatrix::GetCols(LONG nCols1,LONG nCols2)
{
	CMatrix result;
	double *v=new double[m_nNumRows];
	int i=0;
	ASSERT(nCols2>nCols1 && nCols2<=m_nNumColumns);	
	result.Init(m_nNumRows,nCols2-nCols1);
	for (i=nCols1;i<nCols2;i++)
	{
		GetColVector(i,v);
		result.SetCol(i-nCols1,v);
	}
	if (v) delete [] v;
	return result;
}

CMatrix CMatrix::Find(const char *Expr)
{
	int len=strlen(Expr);
	int i=0;
	CMatrix result;
...
CMatrix CMatrix::KronecherProd(CMatrix Y)
{
	CMatrix result;
	CMatrix **Tmp;
	LONG i,j,k,l,m;
	Tmp=(CMatrix **)new CMatrix*[m_nNumRows];
	for (i=0;i<m_nNumRows;i++)
		Tmp[i]=(CMatrix *) new CMatrix[m_nNumColumns];
	double *Vec=(double*) new double[m_nNumColumns*Y.m_nNumColumns];
	double *Vec1=(double *)new double[Y.m_nNumColumns];
	result.Init(m_nNumRows*Y.m_nNumRows,m_nNumColumns*Y.m_nNumColumns);
	for (i=0;i<m_nNumRows;i++)
	{
		for(j=0;j<m_nNumColumns;j++)
		{
			Tmp[i][j]=Y*GetElement(i,j);
		}
	}
	for (i=0;i<m_nNumRows;i++)
	{
		for (j=0;j<m_nNumColumns;j++)
		{
			for (k=(i*Y.m_nNumRows);k<(i+1)*Y.m_nNumRows;k++)
			{
				Tmp[i][j].GetRowVector(k-i*Y.m_nNumRows,Vec1);
				for (l=j*Y.m_nNumColumns,m=0;l<(j+1)*Y.m_nNumColumns;l++,m++)
					result.SetElement(k,l,Vec1[m]);
			}
		}
	}
	if (Vec) delete [] Vec;
...
//mode=0,产生行1距阵
//mode=1,产生列1矩阵
CMatrix CMatrix::Ones(int mode,int nSize1,int nSize2)
{
	ASSERT(nSize1>0);
	CMatrix result;
	if (mode==0)
	{
		if (nSize2==0)
			result.Init(1,nSize1);
		else
			result.Init(nSize1,nSize2);
	}
	else
	{
		if (nSize2==0)
			result.Init(nSize1,1);
		else
			result.Init(nSize1,nSize2);
	}
	if (nSize2==0)
	{
		for (int i=0;i<nSize1;i++)
			if (mode==0)
				result.SetElement(0,i,1.0);
			else
				result.SetElement(i,0,1.0);
	}
	else
	{
		for (int i=0;i<nSize1;i++)
...
void CMatrix::SetRows(CMatrix Tmp, LONG nRows1, LONG nRows2)
{
	ASSERT(m_nNumColumns==Tmp.m_nNumColumns && nRows2>=nRows1);
	LONG i;
	double *v=(double *)new double[m_nNumColumns];
	for (i=nRows1;i<nRows2;i++)
	{
		Tmp.GetRowVector(i-nRows1,v);
		SetRow(i,v);
	}
	if (v) delete [] v;
}

void CMatrix::SetCols(CMatrix Tmp, LONG nCols1, LONG nCols2)
{
	ASSERT(m_nNumRows==Tmp.m_nNumRows && nCols2>=nCols1);
	LONG i;
	double *v=(double *)new double[m_nNumRows];
	for (i=nCols1;i<nCols2;i++)
	{
		Tmp.GetColVector(i-nCols1,v);
		SetCol(i,v);
	}
	if (v) delete [] v;
}

CMatrix CMatrix::GraterOrEqual(double value)
{
	CMatrix result;
	LONG j,total=0;
	ASSERT(m_nNumRows==1);
	...
	{
		if (GetElement(0,j)>=value)
			result.SetElement(0,col++,j);
	}
	return result;
}

CMatrix CMatrix::LessThan(double value)
{
	CMatrix result;
	LONG j,total=0;
	ASSERT(m_nNumRows==1);
...
void CMatrix::SortEigen(double EigenValue[], CMatrix &EigenVector, int mode, int left, int right,BOOL Abs)
{
	if (Abs)
		for (int i=0;i<=right;i++)
			EigenValue[i]=fabs(EigenValue[i]);
	SortEigenVal(EigenValue,EigenVector,mode,left,right);
}
double CMatrix::CalcIntegralImage(LONG x, LONG y)
{
	if (x==-1 || y==-1)
		return 0;
	return	CalcCumulativeSum(x,y)+CalcIntegralImage(x-1,y);
}
double CMatrix::CalcCumulativeSum(LONG x, LONG y)
{
	if (y==-1)
		return 0;
	return CalcCumulativeSum(x,y-1)+GetElement(x,y);
}
double CMatrix::CalcIntegralImageSquare(LONG x, LONG y)
{
	if (x==-1 || y==-1)
		return 0;
	return	CalcCumulativeSumSquare(x,y)+CalcIntegralImageSquare(x-1,y);
}
double CMatrix::CalcCumulativeSumSquare(LONG x, LONG y)
{
	if (y==-1)
		return 0;
	return CalcCumulativeSum(x,y-1)+GetElement(x,y)*GetElement(x,y);
}

CMatrix CMatrix::ChangeSize(LONG height, LONG width)
{
	double xzoom,yzoom;
	LONG i,j,i1,j1;
	CMatrix result(height,width);
	result.Init(height,width);
	xzoom=(double)width/(double)m_nNumColumns;
	yzoom=(double)height/(double)m_nNumRows;
	for (i=0;i<m_nNumRows;i++)
	{
		for (j=0;j<m_nNumColumns;j++)
		{
...
CMatrix CMatrix::CalcIntegralImage()
{
	LONG i,j;
	double value=0;
	CMatrix result;
	...
	for (i=0;i<m_nNumRows;i++)
		for (j=0;j<m_nNumColumns;j++)
		{
			value=CalcIntegralImage(i,j);//-GetElement(i,j);
			result.SetElement(i,j,value);
		}
	return result;
}

CMatrix CMatrix::CalcIntegralImageSquare()
{
	LONG i,j;
	double value=0;
	CMatrix result;
	...
	for (i=0;i<m_nNumRows;i++)
		for (j=0;j<m_nNumColumns;j++)
		{
			value=CalcIntegralImageSquare(i,j);//-GetElement(i,j);
			result.SetElement(i,j,value);
		}
	return result;
}

double CMatrix::CalcIntegralImageRect(LONG x, LONG y, LONG width, LONG height)
{
	if (x==-1 || y==-1)
		return 0;
	if (height==0 || width==0)
		return 0;
	return CalcIntegralImage(x+width,y+height)-CalcIntegralImage(x,y+height)
		-CalcIntegralImage(x+width,y)+CalcIntegralImage(x,y);
}

LONG CMatrix::GetNumTwoRectFeature()
{
	LONG sum=0;
	LONG x,y,w,h,i/*,j*/;
	for (x=1;x<=m_nNumColumns;x++)
	{
		for (y=1;y<=m_nNumRows;y++)
		{
			for (i=1;;i++)
			{
				w=2*i;
				if ((w+x)>m_nNumColumns)
					break;
				//for (j=1;;j++)
				{
				h=i;//pow(2,j);
...
LONG CMatrix::GetNumThreeRectFeature()
{
	LONG sum=0;
	LONG x,y,w,h,i,j;
	for (x=1;x<=m_nNumColumns;x++)
...
LONG CMatrix::GetNumFourRectFeature()
{
	LONG sum=0;
	LONG x,y,w,h,i/*,j*/;
	for (x=1;x<=m_nNumColumns;x++)
	{
		for (y=1;y<=m_nNumRows;y++)
		{
			for (i=0;;i++)
			{
				w=LONG(pow(2.0,i));
				h=w;
				if ((w+x)>m_nNumColumns || (h+y)>m_nNumRows)
					break;
				sum++;
			}
		}
